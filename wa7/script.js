document.addEventListener("DOMContentLoaded",function(){
  ...
  function filterResources(){
    if(!filterInput||!filterList)return;
    var query=filterInput.value.toLowerCase();
    var items=filterList.querySelectorAll("[data-tag]");
    var visible=0;
    ...
    if(filterCount){filterCount.textContent=visible+" result"+(visible===1?"":"s");}
  }
  ...
  function checkForm(e){
    ...
    if(errors.length>0){
      e.preventDefault();
      if(errorBox){
        errorBox.innerHTML="<ul>"+errors.map(function(x){return"<li>"+x+"</li>"}).join("")+"</ul>";
  ...
  if(fabBtn&&fabMenu){
    function openFab(){fabMenu.hidden=false;fabBtn.setAttribute("aria-expanded","true");}
    function closeFab(){fabMenu.hidden=true;fabBtn.setAttribute("aria-expanded","false");}
    fabBtn.addEventListener("click",function(){fabMenu.hidden?openFab():closeFab();});
    fabMenu.addEventListener("click",function(e){if(e.target===fabMenu)closeFab();});
    fabMenu.querySelectorAll("a").forEach(function(a){a.addEventListener("click",closeFab);});
